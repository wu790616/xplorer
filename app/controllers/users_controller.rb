class UsersController < ApplicationController
  ...
  def show
    ...
    following_topics.count.times do |i|
      @map_topics.push({name: following_topics[i].name, base: following_topics[i].id, center: following_topics[i].id, from: following_topics[i].id, page: 0, type: "branch", order: i+5, group: 0})
